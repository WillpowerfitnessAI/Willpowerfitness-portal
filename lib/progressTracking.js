  ...
  async generateProgressReport(userId, timeframe = '30_days') {
    try {
      ...
      const progressAnalysisPrompt = {
        role: "system",
        content: `You are an expert fitness analyst and progress tracking specialist.

        User Progress Data (${timeframe}):
        - Strength Metrics: ${JSON.stringify(strengthData)}
        - Body Composition: ${JSON.stringify(bodyCompData)}
        - Performance Data: ${JSON.stringify(performanceData)}
        - Workout History: ${JSON.stringify(workoutHistory)}

        TASK: Create a comprehensive progress analysis including:
        1. Key achievements and improvements
        2. Areas of concern or plateaus
        3. Trend analysis with specific metrics
        4. Goal progress assessment
        5. Actionable recommendations for next phase
        6. Motivation and acknowledgment of efforts

        Provide detailed insights that justify premium coaching value.`
      };
