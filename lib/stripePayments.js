...
export async function createCheckoutSession(customerId, email, userData) {
  try {
    ...
    const domain = process.env.REPL_URL || 
      (process.env.REPL_SLUG ? 
        `https://${process.env.REPL_SLUG}-${process.env.REPL_OWNER}.replit.app` : 
        'https://willpowerfitnessai.replit.app');
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          ...
          product_data: {
            ...
            description: 'Premium AI Personal Training & Nutrition Coaching',
      ...
      mode: 'subscription',
      success_url: `${domain}/success?session_id={CHECKOUT_SESSION_ID}&customer_id=${customerId}`,
      ...
      custom_text: {
        submit: {
          message: 'Your AI personal trainer will be activated immediately after payment.'
        }
      },
