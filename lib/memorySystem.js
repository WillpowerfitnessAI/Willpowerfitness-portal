...
export async function getUserProfile(userId) {
  try {
    ...
    if (tableCheckError && tableCheckError.code === '42P01') {
      console.log('Tables not found. Please run database setup first.');
      return { id: userId, name: 'New User', goal: 'general_fitness', memory: {} };
    }
    ...
    return data || { id: userId, name: 'New User', goal: 'general_fitness', memory: {} };
  } catch (error) {
...
export async function getConversationHistory(userId, limit = 20) {
  try {
    ...
    if (error) {
      if (error.code === '42P01') {
        console.log('Conversations table not found. Please run database setup first.');
        return [];
      }
      throw error;
    }
