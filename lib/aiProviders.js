...
export async function getChatResponse(messages, userContext = {}) {
  try {
    ...
    const isConsultation = messages.some(msg => 
      msg.content?.includes('consultation') || 
      msg.content?.includes('assessment') ||
      userContext.consultationMode
    );

    if (isConsultation) {
      // Simplified system prompt for consultations to prevent token limit issues
      systemPrompt = {
        role: "system",
        content: `You are Willie, an expert fitness consultant conducting personalized assessments. Be conversational, encouraging, and thorough. Always provide complete responses without cutting off mid-sentence.`
      };
    } else {
      // Full system prompt for regular chat
      systemPrompt = {
        role: "system",
        content: `You are ${userContext.profile?.name || 'your client'}'s Elite AI Personal Trainer with Enhanced Workout Intelligence. 

🧠 ENHANCED CAPABILITIES:
- Dynamic workout adjustments based on performance data
- Advanced form analysis with biomechanical corrections  
- RPE tracking with automatic weight adjustments
- Injury prevention analysis and risk assessment
- Real-time workout modifications

USER CONTEXT: ${JSON.stringify(userContext, null, 2)}

COMMUNICATION: Address them personally, demonstrate advanced AI capabilities, provide scientifically-backed recommendations, and show enthusiasm while maintaining expert-level insights.`
      };
    }
