...
export async function getChatResponse(messages, userContext = {}) {
  try {
    ...
    const isConsultation = userContext.consultationMode === true || 
      (messages.some(msg => 
        msg.content?.includes('consultation') || 
        msg.content?.includes('assessment')
      ) && userContext.chatMode !== 'responsive_trainer');
    ...
    if (isConsultation) {
      ...
      // Responsive chat mode - don't override system prompt if already provided
      if (!messages[0] || messages[0].role !== "system") {
        systemPrompt = {
          role: "system",
          content: `You are a responsive AI personal trainer. Be conversational like ChatGPT but focused on fitness. Respond directly to what the user is saying without launching into consultation mode.`
        };
      } else {
        // System prompt already provided, don't add another
        systemPrompt = null;
      }
    }
